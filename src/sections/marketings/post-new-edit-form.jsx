  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      if (data.coverUrl && data.coverUrl instanceof File) {
        const userId = currentPost?.id || doc(collection(db, 'marketings')).id;
        const fileName = `coverMarketingsUrls/${userId}/${Date.now()}_${data.coverUrl.name}`;
      ...
      const usersRef = collection(db, 'marketings');
      ...
      router.push(paths.dashboard.marketings.list);
